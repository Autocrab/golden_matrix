...
void main() {
  ...
  group('NamingStrategy', () {
    ...
    test('dark theme in path', () {
      final path = NamingStrategy.goldenPath(makeCombination(theme: MatrixTheme.dark));
      expect(path, 'goldens/default/dark_en_ltr_1x_phonesmall.png');
    });

    test('RTL direction in path', () {
      final path = NamingStrategy.goldenPath(
        makeCombination(locale: const Locale('ar'), direction: TextDirection.rtl),
      );
      expect(path, 'goldens/default/light_ar_rtl_1x_phonesmall.png');
    });

    test('text scale 2.0 formatted as 2x', () {
      final path = NamingStrategy.goldenPath(makeCombination(textScale: 2.0));
      expect(path, contains('_2x_'));
    });

    test('text scale 1.3 formatted as 1_3x', () {
      final path = NamingStrategy.goldenPath(makeCombination(textScale: 1.3));
      expect(path, contains('_1_3x_'));
    });

    test('locale with country code', () {
      final path = NamingStrategy.goldenPath(makeCombination(locale: const Locale('en', 'US')));
      expect(path, contains('_en_US_'));
    });

    test('different device in path', () {
      final path = NamingStrategy.goldenPath(makeCombination(device: MatrixDevice.tablet));
      expect(path, 'goldens/default/light_en_ltr_1x_tablet.png');
    });

    test('scenario name in directory', () {
      final path = NamingStrategy.goldenPath(makeCombination(scenario: 'loading'));
    ...
    test('custom theme name is slugified', () {
      ...
      final path = NamingStrategy.goldenPath(makeCombination(theme: customTheme));
